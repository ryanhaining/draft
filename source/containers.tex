...
\pnum
The iterator type for span is a random access iterator and contiguous iterator.
...
\pnum
\requires
\range{firstElem}{lastElem} shall be a valid range.
If \tcode{extent} is not equal to \tcode{dynamic_extent},
then \tcode{lastElem - firstElem} shall be equal to \tcode{extent}.

\pnum
\effects
Constructs a span that is a view over the range \range{firstElem}{lastElem}.
If \tcode{lastElem - firstElem == 0} then an empty \tcode{span} is constructed.

\pnum
\postconditions
\tcode{size() == lastElem - firstElem \&\& data() == firstElem}.
